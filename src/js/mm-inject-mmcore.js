...
;(function () {
  try {
    var settings = JSON.parse(window.localStorage.getItem('mm-inject-settings'));

    if (!window.mmcore) {
      window.mmcore = {
        domain: settings.domain,
        server: settings.srv.replace('?', ''),
        ...
        _async:true,_w:window,_d:document,_undef:undefined,_callback:[],_request_id:0,
        _filters:[],
